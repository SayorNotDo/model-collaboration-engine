...
状态：A+B+C、D 核心反馈指标及统一执行均已合并。当前能力、实际验证与缺口统一见[当前状态](current-status.md)，图形验收见[技术图索引](../diagrams/README.md)。本文保留目标设计与原分析依据，下文旧入口及兼容设想不代表当前契约。
...
后续[榜单接入设计](external-ranking-design.md)补充了独立排序参考：通用 JSON/CSV 的原始名次不填入质量先验；task-fit-v1 仅用它裁决主评分完全相同的候选。
